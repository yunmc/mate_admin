...
import { getRechargeList, getStaffUserInfo } from "@/api/recharge";
...
const getStateStatus = () => {
  return [
    { stateLabel: "MateCoins", stateValue: 0 },
    { stateLabel: "Pro会员", stateValue: 1 }
  ];
};
...
const openDrawer = (title: string, row: Partial<App.rechargeConfig> = {}) => {
  ...
  const params = {
    ...
    isView: title === "查看",
    row: { ...row },
    api: title === "新增" ? getStaffUserInfo : title === "编辑" ? getStaffUserInfo : undefined,
